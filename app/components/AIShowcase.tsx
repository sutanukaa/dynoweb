...
export default function AIShowcaseSection() {
  ...
  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Montserrat:wght@400;500;600;700;800&family=Karla:wght@400;500;600;700&family=Playfair+Display:ital,wght@0,700;1,700&display=swap');
        ...
        @keyframes playPulse {
          0%,100% { box-shadow:0 0 0 0 rgba(59,111,190,.45); }
          50%      { box-shadow:0 0 0 12px rgba(59,111,190,0); }
        }

        .mag-root { font-family:'Karla',sans-serif; }
        ...
        .body-copy { font-size:clamp(.9rem, 1.1vw, 1.6rem); line-height:1.75; color:rgba(255,255,255,0.38); }
        ...
        .mag-bullet:first-child { border-top:1px solid rgba(255,255,255,0.06); }
        .mag-bullet:hover { padding-left:5px; }
        .mag-bullet:hover .mag-bullet-title { color:#6eb0ff; }
        .mag-bullet-title { font-weight:700; font-size:clamp(.8rem, 1vw, 1.4rem); color:rgba(255,255,255,0.82); margin-bottom:2px; transition:color .2s ease; }
        .mag-bullet-desc  { font-size:clamp(.76rem, .9vw, 1.3rem); color:rgba(255,255,255,0.3); line-height:1.5; }
        ...
        /* Glitch / cursor animations unchanged */
        .dw-headline-wrap { position:relative; }
        .dw-glitch-1 { display:inline; animation: dwGlitch1 8s cubic-bezier(.4,0,.2,1) infinite; }
        @keyframes dwGlitch1 {
          0%, 60%   { letter-spacing: 0.04em; }
          68%, 88%  { letter-spacing: -0.02em; }
          96%, 100% { letter-spacing: 0.04em; }
        }
        .dw-glitch-2 { display:inline-block; animation: dwGlitch2 8s cubic-bezier(.4,0,.2,1) infinite; }
        @keyframes dwGlitch2 {
        ...
        .dw-cursor-layer { position:absolute; inset:0; pointer-events:none; z-index:20; overflow:visible; }
        ...
        .dw-cursor {
          position:absolute; width:22px; height:28px; z-index:25;
          filter: drop-shadow(1px 2px 2px rgba(0,0,0,.5));
          animation: dwCursorMove 8s ease infinite;
        }
        @keyframes dwCursorMove {
          0%,10%  { opacity:0; top:20%; right:-30px; }
          22%     { opacity:1; top:55%; right:60%; }
          32%     { opacity:1; top:72%; right:45%; }
          36%     { opacity:1; top:72%; right:45%; transform:scale(0.88); }
          39%     { opacity:1; top:72%; right:45%; transform:scale(1); }
          52%     { opacity:1; top:72%; right:45%; transform:scale(1); }
          62%     { opacity:1; top:78%; right:50%; transform:scale(1); }
          76%     { opacity:0; top:85%; right:60%; transform:scale(1); }
          100%    { opacity:0; top:85%; right:60%; }
        }
        .dw-detect-box {
          position:absolute; top:58%; left:2%; right:8%; height:42%;
          border:1.5px dashed rgba(59,111,190,0.45); border-radius:10px;
          background:rgba(59,111,190,0.05); pointer-events:none; z-index:18;
          animation: dwDetectBox 8s ease infinite;
        }
        @keyframes dwDetectBox {
          0%,34%  { opacity:0; transform:scale(0.96); }
          40%,58% { opacity:1; transform:scale(1); border-color:rgba(59,111,190,0.45); background:rgba(59,111,190,0.05); }
          65%,78% { opacity:1; transform:scale(1); border-color:rgba(34,197,94,0.45); background:rgba(34,197,94,0.04); }
          86%,100%{ opacity:0; transform:scale(0.96); }
        }

        /* Issue/Fixed label — dark version */
        .dw-detect-label {
          ...
          text-transform:uppercase; pointer-events:none; z-index:22;
          white-space:nowrap;
          animation: dwDetectLabel 8s ease infinite;
        }
        @keyframes dwDetectLabel {
          0%,38%  { opacity:0; transform:translateY(6px); color:#6eb0ff; }
          44%,58% { opacity:1; transform:translateY(0);   color:#6eb0ff; }
          62%,64% { opacity:0; transform:translateY(-3px); color:#6eb0ff; }
          65%     { opacity:0; transform:translateY(6px); color:#4ade80; }
          68%,80% { opacity:1; transform:translateY(0);   color:#4ade80; border-color:rgba(74,222,128,0.2); }
          86%,100%{ opacity:0; transform:translateY(-3px); }
        }
        .dw-click-ripple {
          position:absolute; top:70%; right:44%; width:24px; height:24px;
          border-radius:50%; border:1.5px solid rgba(110,176,255,0.4);
        ...
        .dw-issue-text  { animation: dwIssueText  8s ease infinite; }
        @keyframes dwIssueText  { 0%,38% { opacity:0; } 44%,58% { opacity:1; } 62%,100% { opacity:0; } }
        .dw-fixed-text  {
          position:absolute; left:22px; top:50%; transform:translateY(-50%);
          animation: dwFixedText 8s ease infinite;
        }
        @keyframes dwFixedText  { 0%,64% { opacity:0; } 68%,80% { opacity:1; } 86%,100% { opacity:0; } }

        /* Grid */
        .mag-grid { display:flex; flex-direction:column; gap:48px; }
        @media (min-width:640px) {
          .mag-grid {
            display:grid; grid-template-columns:1fr 1fr;
            grid-template-areas:"headline video" "bullets bullets";
          ...
          .mag-bullets-inner { display:grid; grid-template-columns:repeat(3,1fr); gap:0 32px; }
          .mag-bullet { border-top:1px solid rgba(255,255,255,0.06); }
        }
        @media (min-width:1024px) {
          ...
          .mag-bullets-inner { display:block; }
          .mag-bullet { border-top:none; }
          .mag-bullet:first-child { border-top:1px solid rgba(255,255,255,0.06); }
        }
        .mag-col-video .vid-wrap { aspect-ratio:9/16; }
        @media (max-width:639px) { .mag-col-video .vid-wrap { aspect-ratio:4/5; } }
      ...
      <section
        className="mag-root relative w-full overflow-visible"
      ...
      >
        ...
        >
          {/* Section header */}
          <div className="flex flex-col gap-3 mb-16 fi fi-1">
            <span
              style={{
              ...
              <div className="vid-wrap">
                ...
                <div
                  style={{
                    ...
                    inset: 0,
                    background:
                      "linear-gradient(to top,rgba(0,0,0,.85) 0%,rgba(0,0,0,.1) 50%,transparent 75%)",
                ...
                >
                  ...
                  <p
                    style={{
                      fontSize: "clamp(.82rem, 1vw, 1.4rem)",
                      fontWeight: 600,
                      color: "white",
                      lineHeight: 1.4,
                    }}
                ...
                >
                  <span
                    style={{
                      ...
                      height: "6px",
                      borderRadius: "50%",
                      background: playing ? "#22c55e" : "rgba(255,255,255,.25)",
            ...
            >
              <div className="dw-headline-wrap">
                <h2 className="mag-headline">
                  Did you know your{" "}
                  <span className="dw-glitch-1">&lsquo;Buy&rsquo; button</span>{" "}
                  has a 35%{" "}
                ...
                <div className="dw-cursor-layer" aria-hidden="true">
                  ...
                  >
                    <path
                      d="M1 1 L1 19 L5.4 15.2 L8.8 22 L12 20.3 L8.5 13.6 L14 12.5 Z"
              ...
              <p className="body-copy drop-cap">
                ...
                won&rsquo;t complain&mdash;they&rsquo;ll just leave.
                DynoWeb&rsquo;s AI doesn&rsquo;t just show you what happened
                &mdash; it tells you exactly what to do next.
              </p>
              ...
              <div style={{ display: "flex", gap: "24px", flexWrap: "wrap" }}>
                {[
                  {
                    v: "SEO Armor",
                    l: "Every suggestion passes Core Web Vitals validation. Your rankings stay safe.",
                  },
                  {
                    v: "Zero-Risk Drafts",
                    l: "We never touch your live theme. Fixes stay in an isolated Draft Theme.",
                  },
                  {
                    v: "Action, Not Data",
                    l: "Hotjar shows a heatmap. DynoWeb gives you the code to fix it.",
                  },
                ].map((s, i) => (
                  <React.Fragment key={s.v}>
                    ...
                    <div style={{ flex: 1, minWidth: "120px" }}>
                      ...
                      <p
                        style={{
                          fontFamily: "'Karla',sans-serif",
                          fontSize: "clamp(.68rem, .85vw, 1.2rem)",
                          color: "rgba(255,255,255,0.28)",
                          lineHeight: 1.4,
                        }}
                      >
